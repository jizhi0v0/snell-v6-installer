...
- **面向未来**——支持沿用官方命名的 v6 beta、RC/RCN 和正式版，从 `v6.0.0b1` 到 `v6.0.0rc2`、`v6.0.0` 等版本。
...
| 变量 | 作用 | 默认值 |
| --- | --- | --- |
| `VERSION` | 目标版本，如 `v6.0.0b4`、`v6.0.0rc`、`v6.0.0rc2` 或 `v6.0.0`（兼容旧写法 `rc.2`） | 自动解析最新 v6 |
| `SNELL_V6_KNOWN_VERSIONS` | 自动解析时校验的内置已知版本清单，用于 release notes 落后时兜底 | `v6.0.0b1`…`b4`、`v6.0.0rc`、`v6.0.0rc2` |
...
> [!NOTE]
> `VERSION=auto`、`latest`、`arches` 会从官方 release notes **和**内置已知版本清单（`SNELL_V6_KNOWN_VERSIONS`）两处解析，并逐一到下载服务器校验。因此即使 release notes 页面还落后，刚发布的版本（如 `v6.0.0rc2`）也能被识别。版本顺序支持 beta、无编号 RC、带编号 RC 和正式版，例如 `v6.0.0b4 < v6.0.0rc < v6.0.0rc1 < v6.0.0rc2 < v6.0.0`。如果上游发布的版本比两处来源都新，请显式指定 `VERSION`（并把它加入 `SNELL_V6_KNOWN_VERSIONS`）。
