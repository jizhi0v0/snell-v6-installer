...
- **智能版本解析**——不指定 `VERSION` 时自动找到当前 CPU 可用的最新 v6；默认拒绝误降级。
...
通过环境变量传入，例如 `sudo env VERSION=v6.0.0b3 MODE=unshaped ./snell-v6.sh`。

| 变量 | 作用 | 默认值 |
| --- | --- | --- |
| `VERSION` | 目标版本，如 `v6.0.0b3` 或 `v6.0.0` | 自动解析最新 v6 |
...
VERSION=v6.0.0b3 ./snell-v6.sh arches       # 某版本提供的 CPU 包
```
...
`v6.0.0b3` 所需最低客户端版本：**Surge iOS** `5.102.0 (3731)`+，**Surge macOS** `6.7.0-11380`+。
